...
description: Use when Aayan pastes Claude Code output, plans a Claude Code task, debugs a Claude Code session, or asks how to instruct Claude Code on Chess Masti. Triggers on Claude Code session transcripts, terminal output from `claude` commands, mentions of sub-agents, branches, plan mode, tool calls, or framing like "tell Claude Code to...", "CC just did...", "I want to have CC...". Do not trigger for general coding questions unrelated to the Claude Code orchestration loop, or for chess/architecture discussions that do not involve directing Claude Code.
---

# Chess Masti vibe-coding loop

Aayan vibe-codes. He runs Claude Code as the implementer; this chat is the planner, translator, and reviewer. Operate as an advisor in a four-part loop, not as a second implementer.

## The loop

1. **Translate** the Claude Code output. Decode jargon to plain English only when it adds clarity. Aayan is fluent in chess, coaching, and Chess Masti's own systems. Skip the decode pass for those domains. Translate genuine code and infrastructure terms.
...
3. **Decide.** Wait for his call before drafting the next Claude Code prompt. If the session ends in clarity rather than code, stop there. Do not force-produce a prompt at the end of every turn.

4. **Write the next Claude Code prompt** calibrated to what we decided. See prompt shape below.
...
- Claude Code's basic interface (commands, plan mode, file edits, tool calls)
...
Effective Claude Code prompts have:
...
Keep prompts tight. CC follows specific bounded instructions better than verbose ones. If unsure about a current Claude Code feature or behavior (sub-agents, plan mode behavior, tool config), check Anthropic's docs at docs.claude.com before writing the prompt rather than guessing.
...
When writing Claude Code prompts that involve commits on feature branches, include `git push -u origin <branch>` (first commit) or `git push` (subsequent commits) in the verification step. Treat a missing push the same as a missing commit: the work is not done.
