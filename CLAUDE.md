...
- **Assets** live in the versioned, immutable-cached `public/masti/v4/` (six moods: `wave`, `excited`, `idea`, `nervous`, `defeated`, `thinking`; animated WebP at 480 and 240 wide, keyed to transparency from the pack's opaque GIF frames; stills at 640 as WebP + PNG, 1122 as WebP only, and 320 `-sm` as WebP + PNG for avatars). Built from the "Masti Smooth Animations v4" Drive pack by `node scripts/masti/build-assets.mjs --src <unzipped pack>`; never hand-edit the directory. A new pack is a new version directory plus a bump of `MASTI_VERSION` in [src/components/masti/manifest.ts](src/components/masti/manifest.ts); `manifest.test.ts` fails when the TypeScript manifest and the built JSON drift.
- **Components** in [src/components/masti/](src/components/masti/): `Masti` (the figure: still on the server and first paint, animation after mount for `loops` loops, then the still again; never under reduced motion, in a hidden tab, or off screen), `MastiAvatar` (a round face crop for chat headers and verdict pills, stills by default), `MastiSays` (speech bubble), `useStickyMood` (holds a mood for one loop so a 650 ms flash still plays). The `thinking` art carries its own "give me a minute" bubble, so never put a second bubble next to it.
- **Where he is**: home hero and daily puzzle, the analysis coach header and every coach bubble, the puzzle coach panel, verdict pill, streak badge and recap, learn / course / trainer summaries, onboarding tour and nudge, the error boundary and the not-found page (`src/app/not-found.tsx`; the App Router serves every 404 in this mixed app, a `pages/404.tsx` never renders). Each surface computes a mood from its existing state with `puzzleMood` / `analysisMood` / `trendMood` / `scoreMood` and passes it down; do not add a new source of chess truth to do so.
