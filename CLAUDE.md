...
- **Where he is**: home hero and daily puzzle, the analysis coach header and every coach bubble, the puzzle coach panel, verdict pill, streak badge and recap, learn / course / trainer summaries, onboarding tour and nudge, the error boundary and `/404`. Each surface computes a mood from its existing state with `puzzleMood` / `analysisMood` / `trendMood` / `scoreMood` and passes it down; do not add a new source of chess truth to do so.
