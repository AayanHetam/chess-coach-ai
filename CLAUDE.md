...
- **Rules that hold**: keep him out of the under-board strips on the viewport-locked `/analysis` and `/puzzles` layouts (every pixel there is board); use stills below ~48px and for anything in a long transcript; any Masti copy that is a sign-in ask spreads `SIGN_IN_PROPS` (partner preview); keep landing copy clear of the `audiencePositioning.test.tsx` phrase list.
