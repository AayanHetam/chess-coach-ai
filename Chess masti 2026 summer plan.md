...
- **Revenue posture**: zero revenue is fine for the next 18 months — credit-application-driven (Claude for Education being the active path; the OSS path REOPENED 2026-08-19 when the project relicensed to AGPL-3.0).
...
- **OSS strategic path** — UNBLOCKED 2026-08-19: the license posture changed, which was the stated condition for revisiting. Now AGPL-3.0, an OSI-approved license, so Claude for OSS is eligible on license grounds. Remaining gate is its 5k-star threshold (repo is at 0), so the Education path is still the nearer one.
