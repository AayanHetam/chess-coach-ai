...
7. 🟡 **GCC-Eval / ChessQA benchmark.** Scoped in `ACCURACY_BENCHMARK_SCOPE.md`. **First real number (2026-06-12):** ChessQA short-tactics N=15, Claude Sonnet 4 — bare model **0/15 (0%)** → with Stockfish grounding **12/15 (80%)**, **Δ +80pp**. Runner: `scripts/eval/chessqa_grounding_eval.py` (needs the cloned ChessQA repo + a python-chess venv + Stockfish). Caveats: small N; short-tactics is the category where grounding ≈ injecting the engine's best line (large lift expected — it validates the mechanism, not a surprise); OFF=0% is harsh exact-UCI-match (a good-but-not-best move scores 0); ON=80% not 100% because depth-16 SF sometimes disagrees with ChessQA's ground-truth + the model occasionally deviates from the injected line. **STILL TODO:** scale N; add Semantic + Motifs (where grounding must be *reasoned with*, not copied — the more interesting test); Position Judgment (exact-cp, harsh); and **GCC-Eval for the 80% strategic prose** (the calibrated-hedging work is still unmeasured).
