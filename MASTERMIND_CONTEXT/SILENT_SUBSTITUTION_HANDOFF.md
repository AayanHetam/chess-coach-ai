...
1. ~~**P0 legal — AGPL relicense.**~~ **REMEDIATED 2026-08-19 — the license is back to AGPL-3.0.**
   *The finding, for the record:* this repo is a fork of `GuillaumeSD/Chesskit` (earliest commits are his; `upstream` remote confirms). Root `COPYING.md` never stopped stating AGPL-3.0, yet commit `cf90060` (2026-04-18) relicensed to CC BY-NC 4.0 — *"BREAKING CHANGE: License changed from AGPL-3.0 to CC BY-NC 4.0"* — while **100 upstream source files remained in `src/`** (37 byte-identical, 63 modified), including `useEngine`, `useChessActions`, `useGameData`. A fork cannot unilaterally relicense upstream's work.
   *What was done:* `LICENSE` now carries the verbatim AGPL-3.0 text that `COPYING.md` had been pointing at all along, and `package.json` declares `AGPL-3.0-only`. That removes the contradiction at its source rather than papering over it, and it resolves the §13 collision in the right direction: publishing the source is how §13 is satisfied.
   *Still owed by a human:* a courtesy email to GuillaumeSD, and — if the paid tier is ever revived — a lawyer's read. AGPL does **not** block charging for the hosted service; it obliges anyone running a *modified* version as a service to offer their users the source. Detail in the founder's memory file `project_agpl_relicense_exposure.md`.
