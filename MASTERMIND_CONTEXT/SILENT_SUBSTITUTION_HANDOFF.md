...
1. ~~**P0 legal — AGPL relicense.**~~ **RESOLVED 2026-08-19.** This repo grew from `GuillaumeSD/Chesskit` (AGPL-3.0); the 2026-04-18 CC BY-NC relicense of a codebase carrying upstream AGPL code was not sustainable, so the project restored AGPL-3.0-only and went open source (see `LICENSE` and the README's lineage section). AGPL §13's network-source obligation is now satisfied by the public repo itself. Any future commercial plan must stay AGPL-compatible.
...
3. **P1 privacy — account-deletion flow.** `purgeUserData()` covers the tracking tables; the follow-up is an ops deletion script now and a self-serve deletion + data-export flow later, so the deletion path is end-to-end.
4. **P2 — harden the health endpoints.** Add auth/rate limiting to the LLM health-check routes (tracked internally).
