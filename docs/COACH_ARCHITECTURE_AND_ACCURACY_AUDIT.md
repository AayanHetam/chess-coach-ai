...
1. Stockfish analysis of the game;
2. a **context-extraction module** using tactical detectors and models like **Maia** to decide *what is worth explaining at the player's level*;
3. context expressed as a **"data model contract, with every field precisely defined"**;
4. an LLM (they tested Gemini 3 Flash, Claude Opus 4.6, GPT-5 Mini against a ~3-second latency target) generates commentary **only over that structured context**;
5. downvoted outputs trigger **autonomous agent triage with a chess-expert subagent** verifying fixes; SMEs get final say over automated evals.
...
**chesscoach.dev IS Take Take Take — not a separate product.** `chesscoach.dev/faq` issues a 308 permanent redirect to `taketaketake.com`; chesscoach.dev is an SEO/comparison landing domain (pages like `/vs-lichess`, `/faq` comparing against Lichess/Chess.com/DecodeChess/Aimchess) for TTT's game-review feature. This corrects a shared error in the LLM research pass: both ChatGPT and Perplexity treated chesscoach.dev as a distinct indie project, and ChatGPT went further and **fabricated a provenance for it** — a solo-developer "Show HN" backstory with invented quotes ("a mixture of LLMs and engines"). The product *features* it listed (import a Lichess/Chess.com game → 4 "critical moments" → voice-guided plain-English explanation of the mistake and the principle) are real, because they are TTT's features; the *identity and backstory* were confabulated. Chris Butner's open-source `chrisbutner.github.io/ChessCoach` (architecture B, §5.1) is a genuinely separate, unrelated project — do not conflate the three "chess coach" names.
