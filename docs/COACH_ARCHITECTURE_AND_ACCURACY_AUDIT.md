...
*(Sources: primary — official docs, patents, papers, an AI Engineer Europe talk by Take Take Take's team, job postings, direct page fetches; forum — chess.com forums, a TTT teardown. §5.4/§5.6/§5.7 were verified 2026-07-06 via a cross-LLM research pass (ChatGPT + Perplexity) adjudicated against direct web fetches; §5.6's method note explains how single-model hallucinations were filtered out. Full source list in Appendix B.)*
...
**But note the November 2025 pivot**: chess.com's separate **"Play Celebrity Coach"** mode *is* LLM-based — "our chess technology…analyzes the position for every move and gives us a lot of data," an LLM writes the text, **"the team reviews what the LLM produces for accuracy"** (human-in-the-loop verification), plus ElevenLabs voice cloning. So the incumbent's first LLM product shipped with a human accuracy-review layer — further evidence that nobody ships unreviewed LLM chess prose at scale.

### 5.3 DecodeChess

**Architecture A, maximalist**: Stockfish NNUE + patented proprietary XAI that builds "deduction chains" over human concepts (threats, plans, piece functionality), rendered in rich template language. Pre-dates LLMs entirely; still live/marketed but development activity 2024-2026 unverified (possibly dormant). Lesson for us: you can get remarkably rich explanations with zero LLM and zero hallucination — but the language ceiling and dated UI are exactly the gap Chess Masti's UI-as-moat strategy targets.
...
### 5.6 Chessvia and chesscoach.dev (verified 2026-07-06 via a cross-LLM research pass + direct web verification)

**chesscoach.dev now redirects to Take Take Take — but it started as a real indie project and appears to have been absorbed into TTT.** Both `chesscoach.dev/` and `chesscoach.dev/faq` issue a 308 permanent redirect to `taketaketake.com` (verified 2026-07-06), and its live pages (`/vs-lichess`, `/faq` comparing "AI Chess Coach" against Lichess / Chess.com / DecodeChess / Aimchess) carry TTT branding. **Correction to an earlier draft of this section:** I initially wrote that ChatGPT "fabricated" chesscoach.dev's indie backstory. That was wrong — the indie origin is real and triple-corroborated: ChatGPT, Gemini, and Grok all describe it, and Gemini + Grok independently cite the *same* real launch thread ([r/chess "Built a chess coach to help players improve by explaining critical moments"](https://www.reddit.com/r/chess/comments/1lsdik6/), author **gm-ai-agent**, ~mid-2025). So the accurate picture: chesscoach.dev launched as an indie "AI Chess Coach" (import a Lichess/Chess.com game → ~4 "critical moments" → LLM-over-engine-context explanation + a principle lesson) and has since become part of / been rebranded under **Take Take Take** — most likely an acquisition or the founder joining TTT, though the exact nature of the relationship is **not confirmable** from the pages I can fetch (only the redirect + shared branding are certain). Net for competitive purposes: chesscoach.dev is **not an independent competitor today** (it is TTT), but it is *not* a hallucination either — treat it as "TTT, indie-origin." ChatGPT's specific invented *quotes* may still be embellishment, but its provenance claim was broadly correct, not confabulated. Chris Butner's open-source `chrisbutner.github.io/ChessCoach` (architecture B, §5.1) remains a genuinely separate, unrelated project — do not conflate the three "chess coach" names.

**Chessvia** (correct domain **chessvia.ai**, not .com) is a real, live product: an AI coach named **"Chessy"** billed as "the world's first voice-enabled, multi-modal chess AI." Verified facts: voice/text/image interaction during play, Chess.com + Lichess game import, PGN analysis, selectable coach personalities ("Roasty / Grandmaster / Hustler Chessy"), 5 difficulty levels, freemium at **$7–$30/mo** + a limited free tier; listed on AlternativeTo April 2025 (so live by early 2025). **But its architecture is entirely undisclosed** — the engine (Stockfish/Lc0/Maia?), the LLM/vendor, any fine-tuning, and the grounding method are stated nowhere in its public pages or third-party listings. ChatGPT's specific claims about Chessvia — a "Chicago startup," and a Reddit quote ("quickest I've burned 3000 tokens") — could **not** be verified: no such Reddit thread surfaced on a direct search, so treat those as unconfirmed and probably confabulated. Net: Chessvia is confirmed to *exist* and to be voice-first freemium, but its engine/LLM/grounding stack is genuinely **UNKNOWN**, and by disclosure it sits in the "wrapper tail" — architecture-D-shaped marketing with no published verification layer, the same bucket an un-enforced Chess Masti sat in before this week's fixes.

**Methodology note (why the cross-LLM prompt worked):** running the same structured prompt across models with a forced VERIFIED/INFERRED/UNKNOWN split exposed the exact failure it was built to catch. Perplexity (GPT-5, *no* browsing) honestly returned UNKNOWN for every niche target rather than guessing. ChatGPT (*with* browsing) got browsable facts right (TTT's pipeline, Chessvia's voice-first freemium) but confabulated un-browsable provenance (chesscoach.dev's solo dev, Chessvia's Chicago origin, verbatim Reddit quotes). Direct web verification then adjudicated: a claim is trustworthy when it is either primary-sourced or corroborated across an independent model *and* a fetched page — single-model specifics that no page confirms are the hallucination tell. **The competitive claim to earn remains "grounded, verified, and it shows" — the wrapper tail (Chessvia, ChessLogix, ChatGPT wrappers) can't prove it, chess.com stays deliberately generic, and even TTT has the contract but ships unrefereed prose (§5.4/§5.7).**
...
*Reddit was network-blocked during the original 2026-07-05 run. A 2026-07-06 follow-up (cross-LLM research pass + direct web fetches) closed the Chessvia / chesscoach.dev gaps and added the Take Take Take teardown; see §5.4/§5.6/§5.7. Added sources: chessvia.ai + AlternativeTo (Chessvia is voice-first freemium, architecture undisclosed); the chesscoach.dev→taketaketake.com 308 redirect (chesscoach.dev = Take Take Take, not a separate product); taketaketake.com/about + Fast Company + Magnus Carlsen's X announcement (maker = Fantasychess AS + Magnus Carlsen, ~$3M Thiel/Breyer); intermediatemoves.substack.com "TakeTakeTake Game Review is a Slop Machine" (independent architecture confirmation + concrete hallucination examples). Direct r/chess thread mining is still thin — the cross-LLM pass surfaced no verifiable Chessvia Reddit threads, and Grok's X-access sentiment sweep remains the best untried avenue.*
