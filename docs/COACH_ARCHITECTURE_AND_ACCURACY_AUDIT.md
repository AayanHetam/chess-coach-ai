...
*(Sources: primary — official docs, patents, papers, an AI Engineer Europe talk by Take Take Take's team, job postings; forum — chess.com forums. Reddit was network-blocked during the research window; the sentiment section relies on chess.com forums and secondary writeups, flagged accordingly. Full source list in Appendix B.)*
...
### 5.6 Chessvia and chesscoach.dev

Research on these two (plus the Reddit sentiment sweep) was cut short by a network outage during the research window and is queued to re-run; profiles will be appended in a follow-up section. Preliminary signal: both pattern-match to the "wrapper tail" (architecture-D-shaped marketing over engine + frontier LLM, no disclosed verification layer — same bucket as ChessLogix), which is also where an un-enforced Chess Masti effectively sits today. **The competitive claim to earn is "grounded, verified, and it shows" — nobody in the wrapper tail can prove it, chess.com can't say anything interesting, and only TTT currently walks the walk.**

### 5.7 Voice of the user (forums; Reddit re-run pending)
...
*Reddit was network-blocked from the research environment during this run; a re-run is queued and findings will be appended.*
