...
> {
  ...
  for (let i = 0; i < moveHistory.length; i++) {
    ...
    const cpBefore =
      typeof evalBefore.lines[0].mate === "number"
        ? evalBefore.lines[0].mate! > 0
          ? 9999
          : -9999
        : (evalBefore.lines[0].cp ?? 0);
    const cpAfter =
      typeof evalAfter.lines[0].mate === "number"
    ...
    try {
      ...
      console.error(
        `Failed to build puzzles for mistake at move ${Math.floor(i / 2) + 1}:`,
        error
      );
...
export async function POST(request: NextRequest) {
  ...
  return withRequestContext(requestId, async () => {
    ...
    const reportFatal = (
      ...
      logErrorToSentry(err instanceof LLMError ? toSafeLLMError(err) : err, {
    ...
    try {
      ...
      const userRating =
        (isPlausibleRating(userRatingFromBody)
          ? userRatingFromBody
          : undefined) ??
        profileRating ??
      ...
      // no row. Response spans and user identifiers are never persisted.
      const refereeOutcomeBase: Omit<
        RefereeOutcomeContext,
        "model" | "category"
      > = {
      ...
      if (moveHistory && moveHistory.length > 0) {
        ...
        const viewedBlock = buildPositionUnderDiscussion(
          moveHistory,
          viewedPly
        );
        ...
        try {
          ...
          if (tbResult) {
            ...
            if (tbResult.dtm !== null)
              tbBlock += `Distance to mate: ${Math.ceil(Math.abs(tbResult.dtm) / 2)} moves (${Math.abs(tbResult.dtm)} plies)\n`;
      ...
      if (cachedResponse) {
        ...
        try {
          cachedContextId = generateContextId(
            moveHistory,
            fen,
            playerColor || "w",
            session.uid
          ...
          async start(controller) {
            ...
            if (contractForShadowReferee && servingGate.armed) {
              try {
                ...
                const e =
                  err instanceof LLMError ? err : new Error(String(err));
                log.error("Contract-mode serving failed", {
                  message: e.message,
                });
                ...
                send({ type: "error", error: PUBLIC_LLM_ERROR.message });
              }
              controller.close();
              return;
            }
            ...
            if (!prep.dataSources || prep.category === "game_review") {
              ...
              send({
                ...
                metadata: {
                  ...
                  validationIssues:
                    validation.issues.length + enforceableIssues.length,
                  contextId,
                  puzzleRecommendations,
                  corrected:
                    streamCorrected ||
