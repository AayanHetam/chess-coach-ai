...
export const EnhancedAnalysisPanel: React.FC<EnhancedAnalysisPanelProps> = ({
  ...
  const [selectedAnalysisType, setSelectedAnalysisType] =
    useState<ChessAnalysisRequest["analysisType"]>("move_explanation");
  ...
  const formatPositionMetadata = (
    metadata: PositionData["positionMetadata"]
  ) => {
    return {
        ...
        <div className="mt-3 grid grid-cols-3 gap-4 text-sm">
          <div className="text-center">
            <div className="font-semibold text-blue-600">
              {gameState.gameSummary.totalMoves}
            </div>
            <div className="text-gray-500">Moves</div>
          </div>
          <div className="text-center">
            <div className="font-semibold text-green-600">
              {gameState.gameSummary.totalPositions}
            </div>
            <div className="text-gray-500">Positions</div>
          </div>
          <div className="text-center">
            <div className="font-semibold text-purple-600">
              {gameState.gameSummary.gameResult}
            </div>
        ...
        <div className="p-4 space-y-4">
          ...
          {gameState.currentPosition && (
            <div className="bg-gray-50 p-3 rounded-lg">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                {Object.entries(
                  formatPositionMetadata(
                    gameState.currentPosition.positionMetadata
                  )
                ).map(([key, value]) => (
          ...
          <div className="space-y-3">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedAnalysisType(
                    e.target.value as ChessAnalysisRequest["analysisType"]
                  )
                }
            ...
            <button
              ...
              disabled={
                isLoading || !openAIApiKey || gameState.allPositions.length < 2
              }
                ...
                {gameState.allPositions.slice(-5).map((pos, index) => (
                  ...
                  >
                    <span className="font-mono">
                      {pos.moveNumber}.
                      {pos.halfMoveNumber % 2 === 1 ? "" : ".."}
                      {pos.movePlayed?.san || "Start"}
                    </span>
                    <span className="text-gray-500">
                      {pos.positionMetadata.gamePhase}
                    </span>
                  </div>
                ))}
              </div>
