...
const EvaluationBar: React.FC<EvaluationBarProps> = ({
  ...
  const normalizedEvaluation = isMate
    ? maxEvaluation
    : Math.min(Math.abs(evaluation), maxEvaluation);
  ...
  const formatEvaluation = (evalValue: number) => {
    if (isMate) {
      const mateValue =
        Math.abs(evalValue) === 10000 ? 1 : Math.abs(evalValue) / 10000;
      return `M${mateValue}`;
    }
  ...
  const progressPercentage = isWhiteBetter
    ? 50 + percentage / 2
    : 50 - percentage / 2;
