...
import { analyzeGameSurprises, SurpriseAnalysis } from '../engine/surpriseAnalyzer';
...
export async function analyzeGameAggressively(
  ...
  const movePurposeAnalysis = await analyzeMovePurposes(gameHistory, userColor, engineAnalysis);
  
  // Get surprise analysis (from chess-surprise-analysis repository)
  const surpriseAnalysis = await analyzeGameSurprises(gameHistory, userColor, engineAnalysis);
  ...
  for (let i = 0; i < gameHistory.length; i++) {
    ...
    const evaluationChange = await calculateEvaluationChangeWithEngine(move, positionBefore, positionAfter, userColor);
    
    // Debug output for significant evaluation changes
    if (Math.abs(evaluationChange) > 0.5) {
      console.log(`Move ${moveNumber}. ${move}: evaluation change = ${evaluationChange.toFixed(2)}`);
    }
    
    // Debug output for user moves that don't get violations
    if (isUserMove && evaluationChange >= -0.3) {
      console.log(`Move ${moveNumber}. ${move}: evaluation change = ${evaluationChange.toFixed(2)} (no violation - too small)`);
    }
    ...
    const allViolations = (evaluationChange < -0.3 && isUserMove) ? 
      generateViolationsForWorstMove(move, moveNumber, positionBefore, positionAfter, gameHistory.slice(0, i), evaluationChange) :
      [];
    
    // Debug output for violations
    if (isUserMove) {
      console.log(`Move ${moveNumber}. ${move}: evaluation change = ${evaluationChange.toFixed(3)}, violations = ${allViolations.length}`);
      if (allViolations.length > 0) {
        console.log(`  🚨 Violations found:`, allViolations.map(v => `${v.principle.name} (${v.severity})`));
      } else if (evaluationChange < -0.3) {
        console.log(`  ⚠️  No violations generated despite significant evaluation drop`);
      }
    }
    
    // Get move purpose information
    const movePurpose = movePurposeAnalysis.moves.find(mp => mp.move === move && mp.moveNumber === moveNumber);
    
    // Get surprise analysis information
    const surpriseAnalysisForMove = surpriseAnalysis.moves.find(sa => sa.move === move && sa.moveNumber === moveNumber);
...
async function calculateEvaluationChangeWithEngine(
  ...
  try {
    ...
    console.error('❌ Engine evaluation failed, using basic evaluation:', error);
    // Fallback to basic evaluation if engine fails
    const basicEval = calculateBasicEvaluationChange(move, positionBefore, positionAfter, userColor);
...
function calculateBasicEvaluationChange(
  ...
  const totalChange = materialChange + 
...
function countTacticalOpportunities(position: Chess): number {
  ...
  if (enemyKingSquare) {
    opportunities += moves.filter(m => 
      m.includes(enemyKingSquare) || 
      isNearKing(m, enemyKingSquare)
    ).length;
  }
...
function isNearKing(move: string, kingSquare: string): boolean {
  ...
  if (targetSquare) {
    ...
    return Math.abs(targetFile - kingFile) <= 2 && Math.abs(targetRank - kingRank) <= 2;
  }
...
function generateViolationsForWorstMove(
  ...
  console.log(`🔍 Generating violations for move ${moveNumber}. ${move} (eval change: ${evaluationChange.toFixed(3)})`);
  ...
  const mostRelevantPrinciple = findMostRelevantPrincipleForMove(move, moveNumber, gamePhase, evaluationChange);
  console.log(`  🎯 Most relevant principle: ${mostRelevantPrinciple?.name || 'None found'}`);
  ...
  if (mostRelevantPrinciple) {
    const violation = {
      ...
      shortTermImpact: getShortTermImpact(mostRelevantPrinciple, evaluationChange),
      longTermImpact: getLongTermImpact(mostRelevantPrinciple, evaluationChange),
      correctMove: suggestCorrectMove(positionBefore, moveNumber, mostRelevantPrinciple)
    };
...
function findMostRelevantPrincipleForMove(
  ...
  const specificMistake = identifySpecificMistake(move, moveNumber, gamePhase, evaluationChange);
...
function identifySpecificMistake(
  ...
  if (evaluationChange < -0.8) {
    ...
    if (move.includes('x') || move.includes('+') || move.includes('Q') || move.includes('R')) {
      return 'missed-tactical-opportunity';
    }
  ...
  if (gamePhase === 'opening' && moveNumber <= 10 && evaluationChange < -0.3) {
    ...
    if (move.includes('p') && (move.includes('a3') || move.includes('h3') || move.includes('a6') || move.includes('h6'))) {
      return 'pushed-edge-pawns';
    }
    
    // Don't block central pawns
    if (move.includes('p') && (move.includes('d3') || move.includes('e3') || move.includes('d6') || move.includes('e6'))) {
  ...
  if (gamePhase === 'middlegame' && evaluationChange < -0.5) {
    ...
    if (move.includes('p') && (move.includes('f3') || move.includes('g3') || move.includes('h3') || 
                               move.includes('f6') || move.includes('g6') || move.includes('h6'))) {
  ...
  if (gamePhase === 'endgame') {
    ...
    if ((move.includes('N') || move.includes('B')) && move.includes('x') && evaluationChange < -0.5) {
...
function mapMistakeToPrinciple(mistake: string, gamePhase: 'opening' | 'middlegame' | 'endgame'): ChessPrinciple | null {
  ...
  if (mistake === 'missed-tactical-opportunity' || mistake === 'allowed-opponent-trade') {
    return generalPrinciples.find(p => p.id === 'look-for-basic-tactics') || null;
  }
  ...
  if (gamePhase === 'opening') {
    switch (mistake) {
      ...
      case 'exposed-queen-early':
        return openingPrinciples.find(p => p.id === 'dont-bring-queen-early') || null;
      case 'pushed-edge-pawns':
      case 'blocked-central-pawns':
      case 'created-pawn-weaknesses':
        return openingPrinciples.find(p => p.id === 'limit-pawn-moves') || null;
      case 'over-moved-piece':
        return openingPrinciples.find(p => p.id === 'avoid-moving-same-piece') || null;
      case 'ignored-opponent-threats':
      case 'allowed-easy-pin':
      case 'launched-premature-attack':
        return generalPrinciples.find(p => p.id === 'look-for-basic-tactics') || null;
      default:
        return openingPrinciples.find(p => p.id === 'develop-knights-bishops') || null;
    }
  }
  ...
  if (gamePhase === 'middlegame') {
    switch (mistake) {
      case 'left-piece-undefended':
        return generalPrinciples.find(p => p.id === 'dont-hang-pieces') || null;
      case 'allowed-fork-or-pin':
        return generalPrinciples.find(p => p.id === 'look-for-basic-tactics') || null;
      case 'weakened-king-shield':
        return middlegamePrinciples.find(p => p.id === 'maintain-king-safety') || null;
      case 'traded-into-worse-position':
      case 'ignored-open-file':
      case 'overextended-pawns':
        return generalPrinciples.find(p => p.id === 'assess-pawn-structure') || null;
      case 'trapped-major-piece':
      case 'blocked-own-pieces':
        return middlegamePrinciples.find(p => p.id === 'maximize-piece-activity') || null;
  ...
  if (gamePhase === 'endgame') {
    switch (mistake) {
      ...
      case 'placed-rook-in-front-of-pawn':
        return endgamePrinciples.find(p => p.id === 'support-passed-pawns') || null;
      case 'traded-into-losing-endgame':
      case 'traded-minor-pieces-carelessly':
        return endgamePrinciples.find(p => p.id === 'avoid-bad-trades') || null;
      case 'missed-zugzwang-opportunity':
        return endgamePrinciples.find(p => p.id === 'create-zugzwang') || null;
...
function getSeverityFromEvaluationChange(evaluationChange: number): 'minor' | 'moderate' | 'major' {
...
function getShortTermImpact(principle: ChessPrinciple, evaluationChange: number): string {
  const absChange = Math.abs(evaluationChange);
  ...
  switch (principle.id) {
    ...
    case 'look-for-basic-tactics':
      return absChange >= 1.0 ? 'Missed tactical advantage' : 'Missed opportunity';
...
function getLongTermImpact(principle: ChessPrinciple, evaluationChange: number): string {
  switch (principle.id) {
    case 'dont-hang-pieces':
...
function suggestCorrectMove(position: Chess, moveNumber: number, principle: ChessPrinciple): string {
  switch (principle.id) {
    case 'dont-hang-pieces':
...
function suggestTacticalMove(position: Chess, moveNumber: number): string {
  ...
  const threatMoves = moves.filter(m => 
...
function allowsOpponentTrade(move: string): boolean {
  ...
  return move.includes('p') && (move.includes('c5') || move.includes('c4') || move.includes('d5') || move.includes('d4'));
}
