...
export function analyzeEndgamePrinciples(
  ...
  if (kingActivation.applied) {
    appliedPrinciples.push(endgamePrinciples.find(p => p.id === 'activate-king')!);
  ...
  if (passedPawns.hasPassedPawn) {
    appliedPrinciples.push(endgamePrinciples.find(p => p.id === 'create-outside-passed-pawns')!);
    
    if (passedPawns.isSupported) {
      appliedPrinciples.push(endgamePrinciples.find(p => p.id === 'support-passed-pawns')!);
    } else {
      violatedPrinciples.push({
        principle: endgamePrinciples.find(p => p.id === 'support-passed-pawns')!,
        severity: 'moderate',
  ...
  if (blockade.applied) {
    appliedPrinciples.push(endgamePrinciples.find(p => p.id === 'blockade-opponent-pawns')!);
  } else if (blockade.violated) {
    violatedPrinciples.push({
      principle: endgamePrinciples.find(p => p.id === 'blockade-opponent-pawns')!,
      severity: 'major',
  ...
  if (pawnMajority.applied) {
    appliedPrinciples.push(endgamePrinciples.find(p => p.id === 'use-pawn-majorities')!);
  }
  ...
  if (simplification.shouldSimplify && !simplification.isSimplifying) {
    ...
    appliedPrinciples.push(endgamePrinciples.find(p => p.id === 'simplify-when-ahead')!);
  }
  
  return { appliedPrinciples, violatedPrinciples };
}

function analyzeKingActivation(position: Chess): { applied: boolean; violated: boolean; description: string } {
  const turn = position.turn();
  ...
  const isCentralized = (kingFile >= 2 && kingFile <= 5) && (kingRank >= 3 && kingRank <= 4);
...
function analyzePassedPawns(position: Chess): { hasPassedPawn: boolean; isSupported: boolean } {
  const turn = position.turn();
...
function analyzeBlockade(position: Chess): { applied: boolean; violated: boolean; description: string } {
  const turn = position.turn();
  const opponentColor = turn === 'w' ? 'b' : 'w';
  const opponentPassedPawns = findPassedPawns(position, opponentColor);
  
  if (opponentPassedPawns.length === 0) {
    return { applied: false, violated: false, description: 'No opponent passed pawns to blockade' };
  }
  ...
  return {
    ...
    description: unblockedDangerous > 0 ? 
...
function analyzePawnMajorities(position: Chess): { applied: boolean } {
  ...
  const isAdvancingMajority = checkPawnMajorityAdvancement(position, turn, hasKingsideMajority, hasQueensideMajority);
  
  return {
    applied: (hasKingsideMajority || hasQueensideMajority) && isAdvancingMajority
  };
}

function analyzeSimplification(position: Chess, evaluation: PositionEval): { shouldSimplify: boolean; isSimplifying: boolean } {
...
function checkKingActivity(position: Chess, kingSquare: string, color: 'w' | 'b'): boolean {
...
function isPawnPassed(position: Chess, pawnSquare: string, color: 'w' | 'b'): boolean {
  ...
  for (const fileOffset of [-1, 0, 1]) {
    ...
    for (let checkRank = startRank; color === 'w' ? checkRank <= endRank : checkRank >= endRank; checkRank += direction) {
      const checkSquare = String.fromCharCode(97 + checkFile) + (checkRank + 1);
      const piece = position.get(checkSquare as any);
...
function isPawnSupported(position: Chess, pawnSquare: string, color: 'w' | 'b'): boolean {
  const file = pawnSquare.charCodeAt(0) - 97;
  const rank = parseInt(pawnSquare[1]) - 1;
  ...
  for (const square of supportingSquares) {
    if (square[0] < 'a' || square[0] > 'h' || square[1] < '1' || square[1] > '8') continue;
    
    const piece = position.get(square as any);
    if (piece && piece.color === color && (piece.type === 'k' || piece.type === 'r' || piece.type === 'p')) {
      return true;
    }
  }
...
function checkPawnMajorityAdvancement(position: Chess, color: 'w' | 'b', hasKingsideMajority: boolean, hasQueensideMajority: boolean): boolean {
  ...
  for (let rank = 0; rank < 8; rank++) {
    for (let file = 0; file < 8; file++) {
      ...
      if (piece && piece.type === 'p' && piece.color === color) {
        ...
        if ((hasKingsideMajority && wing === 'kingside') || (hasQueensideMajority && wing === 'queenside')) {
          const actualRank = rank + 1;
          if (color === 'w' ? actualRank > mostAdvancedRank : actualRank < mostAdvancedRank) {
