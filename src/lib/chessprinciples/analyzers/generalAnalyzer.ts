...
export function analyzeGeneralPrinciples(
  ...
  if (hangingPieces.violated) {
    ...
    appliedPrinciples.push(generalPrinciples.find(p => p.id === 'dont-hang-pieces')!);
  }
  ...
  if (basicTactics.missedOpportunity) {
    violatedPrinciples.push({
      principle: generalPrinciples.find(p => p.id === 'look-for-basic-tactics')!,
      severity: 'moderate',
      description: basicTactics.description,
      shortTermImpact: 'Missed tactical opportunity for material gain or advantage',
      longTermImpact: 'Accumulation of missed tactics reduces winning chances'
    });
  } else if (basicTactics.applied) {
    appliedPrinciples.push(generalPrinciples.find(p => p.id === 'look-for-basic-tactics')!);
  }
  
  // 3. Maintain material balance
  const materialBalance = analyzeMaterialBalance(position, evaluation);
  if (materialBalance.applied) {
    appliedPrinciples.push(generalPrinciples.find(p => p.id === 'maintain-material-balance')!);
  } else if (materialBalance.violated) {
    violatedPrinciples.push({
      principle: generalPrinciples.find(p => p.id === 'maintain-material-balance')!,
      severity: materialBalance.severity,
      description: materialBalance.description,
      shortTermImpact: 'Material disadvantage affects immediate tactical options',
  ...
  if (spaceControl.applied) {
    appliedPrinciples.push(generalPrinciples.find(p => p.id === 'control-space')!);
  ...
  if (pawnStructure.hasWeaknesses) {
    violatedPrinciples.push({
      principle: generalPrinciples.find(p => p.id === 'assess-pawn-structure')!,
      severity: 'moderate',
    ...
    appliedPrinciples.push(generalPrinciples.find(p => p.id === 'assess-pawn-structure')!);
  }
  ...
  if (tacticalAwareness.applied) {
    appliedPrinciples.push(generalPrinciples.find(p => p.id === 'calculate-accurately')!);
  } else if (tacticalAwareness.violated) {
    violatedPrinciples.push({
      principle: generalPrinciples.find(p => p.id === 'calculate-accurately')!,
...
function analyzeMaterialBalance(position: Chess, evaluation: PositionEval): { 
  applied: boolean; 
  violated: boolean; 
  ...
  const hasRecentSacrifice = evaluation.lines?.[0]?.pv?.some(move => 
    position.history({ verbose: true }).some(histMove => 
  ...
  if (balance >= 0) {
    return {
      ...
      violated: false,
      severity: 'minor',
      description: balance > 0 ? `Material advantage: +${balance}` : 'Material equality maintained'
    };
  }
  
  const severity: 'minor' | 'moderate' | 'major' = 
    Math.abs(balance) <= 1 ? 'minor' :
    Math.abs(balance) <= 3 ? 'moderate' : 'major';
...
function analyzeSpaceControl(position: Chess): { applied: boolean; violated: boolean; description: string } {
  const turn = position.turn();
  ...
  return {
    ...
    description: !spaceAdvantage && centralControl === 0 ?
...
function analyzePawnStructure(position: Chess): { hasWeaknesses: boolean; isGood: boolean; description: string } {
  const turn = position.turn();
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (pawnFiles[file] && pawnFiles[file].length > 0) {
      ...
      const hasLeftSupport = leftFile && pawnFiles[leftFile] && pawnFiles[leftFile].length > 0;
      const hasRightSupport = rightFile && pawnFiles[rightFile] && pawnFiles[rightFile].length > 0;
  ...
  const hasSignificantWeaknesses = isolatedPawns > 1 || doubledPawns > 1 || pawnIslands > 3;
  const hasMinorWeaknesses = isolatedPawns === 1 || doubledPawns === 1;
  const isGood = !hasSignificantWeaknesses && !hasMinorWeaknesses && pawnIslands <= 2;
...
function analyzeTacticalAwareness(position: Chess, evaluation: PositionEval): {
  applied: boolean;
  violated: boolean;
  severity: 'minor' | 'moderate' | 'major';
  description: string;
  ...
  const tacticalThreats = [canCaptureForFree, hasDiscoveredAttacks, hasForks, hasPins].filter(Boolean).length;
...
function checkForPins(position: Chess): boolean {
  ...
  for (let rank = 0; rank < 8; rank++) {
    for (let file = 0; file < 8; file++) {
      const piece = board[rank][file];
      if (piece && piece.color === turn && (piece.type === 'b' || piece.type === 'r' || piece.type === 'q')) {
  ...
  for (let rank = 0; rank < 8; rank++) {
    for (let file = 0; file < 8; file++) {
      ...
      if (piece && piece.color === turn) {
        ...
        const isAttacked = isPieceAttacked(position, square, turn === 'w' ? 'b' : 'w');
...
function analyzeBasicTactics(position: Chess): {
  ...
  const opponentHangingPieces = findHangingPieces(position, turn === 'w' ? 'b' : 'w');
  if (opponentHangingPieces.length > 0) {
    tacticalOpportunities.push(`Can capture hanging pieces: ${opponentHangingPieces.join(', ')}`);
  }
...
function isPieceDefended(position: Chess, square: string, color: string): boolean {
...
function isPieceAttacked(position: Chess, square: string, byColor: string): boolean {
  ...
  for (let rank = 0; rank < 8; rank++) {
    for (let file = 0; file < 8; file++) {
      ...
      if (piece && piece.color === color) {
        ...
        const isAttacked = isPieceAttacked(position, square, color === 'w' ? 'b' : 'w');
