...
export function analyzeMiddlegamePrinciples(
  ...
  if (pieceActivity.applied) {
    appliedPrinciples.push(
      middlegamePrinciples.find((p) => p.id === "maximize-piece-activity")!
    );
  } else if (pieceActivity.violated) {
    violatedPrinciples.push({
      principle: middlegamePrinciples.find(
  ...
  if (worstPiece.applied) {
    appliedPrinciples.push(
      middlegamePrinciples.find((p) => p.id === "improve-worst-piece")!
    );
  } else if (worstPiece.violated) {
    violatedPrinciples.push({
      principle: middlegamePrinciples.find(
        (p) => p.id === "improve-worst-piece"
      )!,
      severity: "moderate",
  ...
  if (centralization.applied) {
    appliedPrinciples.push(
      middlegamePrinciples.find((p) => p.id === "centralize-pieces")!
    );
  } else if (centralization.violated) {
    violatedPrinciples.push({
      principle: middlegamePrinciples.find(
        (p) => p.id === "centralize-pieces"
      )!,
      severity: "minor",
  ...
  if (prophylacticThinking.applied) {
    appliedPrinciples.push(
      middlegamePrinciples.find((p) => p.id === "think-prophylactically")!
    );
  } else if (prophylacticThinking.violated) {
    violatedPrinciples.push({
      principle: middlegamePrinciples.find(
        (p) => p.id === "think-prophylactically"
      )!,
      severity: "minor",
  ...
  if (fileControl.applied) {
    appliedPrinciples.push(
      middlegamePrinciples.find((p) => p.id === "control-open-files")!
    );
  } else if (fileControl.violated) {
    violatedPrinciples.push({
      principle: middlegamePrinciples.find(
        (p) => p.id === "control-open-files"
      )!,
      severity: "moderate",
  ...
  if (outposts.applied) {
    appliedPrinciples.push(
      middlegamePrinciples.find((p) => p.id === "knights-on-outposts")!
    );
  }
  ...
  if (bishopDiagonals.applied) {
    appliedPrinciples.push(
      middlegamePrinciples.find((p) => p.id === "bishops-long-diagonals")!
    );
  }
  ...
  if (keySquares.applied) {
    appliedPrinciples.push(
      middlegamePrinciples.find((p) => p.id === "control-key-squares")!
    );
  } else if (keySquares.violated) {
    violatedPrinciples.push({
      principle: middlegamePrinciples.find(
        (p) => p.id === "control-key-squares"
      )!,
      severity: "moderate",
...
function analyzePieceActivity(position: Chess): {
  applied: boolean;
  violated: boolean;
  description: string;
} {
  ...
  for (const piece of pieces) {
    if (
      ...
      const pieceMoves = moves.filter(
        (move) => move.from === getSquareFromPiece(piece, position)
      );
...
function analyzeCentralization(position: Chess): {
  applied: boolean;
  violated: boolean;
  description: string;
} {
...
function analyzeFileControl(position: Chess): {
  applied: boolean;
  violated: boolean;
  description: string;
} {
...
function analyzeKingSafety(position: Chess): {
  applied: boolean;
  violated: boolean;
  description: string;
} {
  ...
  const isSafe =
    (isLikelyCastled || hasGoodPawnShield) && !kingInCheck && threatCount < 2;
  const isUnsafe =
    kingInCheck || threatCount > 2 || (!isLikelyCastled && !hasGoodPawnShield);
...
function analyzeKeySquareControl(position: Chess): {
  applied: boolean;
  violated: boolean;
  description: string;
} {
...
function isSquareSafeFromPawns(
  position: Chess,
  square: string,
  color: "w" | "b"
): boolean {
  ...
  if (attackRank >= 0 && attackRank < 8) {
    for (const fileOffset of [-1, 1]) {
      ...
      if (attackFile >= 0 && attackFile < 8) {
        const attackSquare =
          String.fromCharCode(97 + attackFile) + (attackRank + 1);
...
function getLongestDiagonalLength(square: string): number {
  ...
  return Math.max(toTopRight + toBottomLeft + 1, toTopLeft + toBottomRight + 1);
}

function getDefendingMoves(
  position: Chess,
  square: string,
  color: "w" | "b"
): any[] {
...
function analyzeWorstPiece(position: Chess): {
  ...
  const worstPieceEntry = Object.entries(pieceActivity).reduce(
...
function calculatePieceActivity(
  ...
  const file = square.charCodeAt(0) - 97;
  const rank = parseInt(square[1]) - 1;
  const centralBonus = Math.max(
    0,
    3 - Math.abs(3.5 - file) - Math.abs(3.5 - rank)
  );
