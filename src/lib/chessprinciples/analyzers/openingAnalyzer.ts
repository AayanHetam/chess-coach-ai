...
export function analyzeOpeningPrinciples(
  ...
  if (centerControl.applied) {
    appliedPrinciples.push(
      openingPrinciples.find((p) => p.id === "control-center-pawns")!
    );
  } else if (centerControl.violated) {
    violatedPrinciples.push({
      principle: openingPrinciples.find(
        (p) => p.id === "control-center-pawns"
      )!,
      severity: "moderate",
      description:
        "Limited central pawn presence - consider moves like e4, d4, e5, or d5",
  ...
  if (development.applied) {
    ...
    violatedPrinciples.push({
      principle: openingPrinciples.find(
        (p) => p.id === "develop-knights-bishops"
      )!,
      severity: "moderate",
      description: development.description,
      shortTermImpact: "Slower piece coordination and initiative loss",
      longTermImpact:
        "Opponent completes development first, gaining tempo advantage",
    });
  }
  ...
  if (completeDevelopment.applied) {
    appliedPrinciples.push(
      openingPrinciples.find((p) => p.id === "complete-development-first")!
    );
  } else if (completeDevelopment.violated) {
    violatedPrinciples.push({
      principle: openingPrinciples.find(
        (p) => p.id === "complete-development-first"
      )!,
      severity: "moderate",
  ...
  if (repetitiveMoves.violated) {
    violatedPrinciples.push({
      principle: openingPrinciples.find(
        (p) => p.id === "avoid-moving-same-piece"
      )!,
      severity: "minor",
  ...
  if (castling.applied) {
    appliedPrinciples.push(
      openingPrinciples.find((p) => p.id === "castle-early")!
    );
    ...
    violatedPrinciples.push({
      ...
      longTermImpact:
        "Difficulty coordinating rooks and maintaining king safety",
    });
  }
...
function analyzeCenterControl(position: Chess): {
  applied: boolean;
  violated: boolean;
} {
  const fen = position.fen();
  ...
  const attacks = position
    .moves({ verbose: true })
    .filter((move) => centralSquares.includes(move.to) && move.piece === "p");
...
function analyzePieceDevelopment(
  position: Chess,
  history: string[]
): { applied: boolean; violated: boolean; description: string } {
  const board = position.board();
  const turn = position.turn();
  ...
  const developmentRatio =
    totalMinorPieces > 0 ? developedMinorPieces / totalMinorPieces : 0;
...
function analyzeRepetitiveMoves(history: string[]): {
  violated: boolean;
  description: string;
} {
  ...
  const excessivePiece = Object.entries(pieceMoveCounts).find(
    ([_, count]) => count === maxMoves
  )?.[0];
...
function analyzeCastling(
  ...
  const moveCount = history.length;
  const halfMoveCount =
    turn === "w" ? Math.ceil(moveCount / 2) : Math.floor(moveCount / 2);
  ...
  if (hasCastled) {
    return {
      ...
      violated: false,
      severity: "minor",
      description: "Successfully castled",
    };
  }
  ...
  return {
    ...
    violated: false,
    severity: "minor",
    description: "Castling still available",
  };
}
...
function analyzePawnMoves(history: string[]): {
  violated: boolean;
  description: string;
} {
  const recentMoves = history.slice(-8);
  const pawnMoves = recentMoves.filter(
    (move) => !move.match(/^[NBRQK]/)
  ).length;
...
function analyzeCompleteDevelopment(
  ...
  const developmentRatio =
    totalMinorPieces > 0 ? developedMinorPieces / totalMinorPieces : 1;
  ...
  if (currentPlayerMoves <= 15 && isDevelopmentComplete) {
    return {
      ...
      violated: false,
      description:
        "Good development - pieces developed and king castled before major attacks",
    };
  }
