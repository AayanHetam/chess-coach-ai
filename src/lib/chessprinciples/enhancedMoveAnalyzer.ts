...
async function analyzeSingleMoveEnhanced(
  ...
  const movePurpose = determineMovePurpose(move, surpriseScore, lowDepthEval, highDepthEval);
  ...
  const explanation = generateComprehensiveExplanation(
    move, moveNumber, movePurpose, surpriseScore, bestMove, 
    tacticalThemes, positionInsights, pieceSquareAnalysis
  );
    ...
    for (let j = 0; j < 8; j++) {
      ...
      if (piece && (piece.type === 'r' || piece.type === 'q')) {
        ...
        if (isPinnedPiece(position, square)) {
          pinned.push(`${piece.color ? 'White' : 'Black'} ${piece.type} at ${getSquareName(square)} is pinned`);
        }
      }
    }
  }
  ...
  for (let i = 0; i < 8; i++) {
    for (let j = 0; j < 8; j++) {
      ...
      if (piece && isTrappedPiece(position, square)) {
        trapped.push(`${piece.color ? 'White' : 'Black'} ${piece.type} at ${getSquareName(square)} is trapped`);
      }
    }
  }
...
function analyzeMobility(position: Chess): string[] {
  ...
  if (whiteMoves > blackMoves) {
    analysis.push(`White has better mobility (${whiteMoves} vs ${blackMoves} moves)`);
  } else if (blackMoves > whiteMoves) {
    analysis.push(`Black has better mobility (${blackMoves} vs ${whiteMoves} moves)`);
  } else {
...
function generateEnhancedAssessment(moves: EnhancedMoveAnalysis[]): string {
  ...
  const positionalMistakes = moves.filter(m => m.movePurpose === 'positional' && m.surpriseScore > 0.5).length;
...
async function getEngineEvaluation(position: Chess, depth: number): Promise<number> {
