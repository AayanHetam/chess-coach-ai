            ...
            if (
              ...
              moveAnalysis.principleViolations.push({
                principle: {
                  ...
                  description: "Develop knights and bishops before pawn moves",
                  category: "opening",
                  priority: 8,
                },
            ...
            if (move.includes("Q") && moveNumber <= 6) {
              moveAnalysis.principleViolations.push({
                principle: {
                  ...
                  description: "Develop minor pieces before the queen",
                  category: "opening",
                  priority: 7,
                },
...
function analyzeSingleMove(
  ...
  for (const principle of applicablePrinciples) {
    const analysis = analyzePrincipleForMove(
      principle,
      positionBefore,
      move,
      moveNumber,
  ...
  const assessment = determineMoveAssessment(
    principleViolations,
    missedOpportunities
  );
...
function getAllApplicablePrinciples(
  gamePhase: "opening" | "middlegame" | "endgame"
): ChessPrinciple[] {
...
function analyzePrincipleForMove(
  ...
  try {
    ...
    switch (principle.category) {
      case "opening":
        return analyzeOpeningPrinciple(
          principle,
          positionBefore,
          positionAfter,
          move,
      ...
      case "middlegame":
        return analyzeMiddlegamePrinciple(
          principle,
          positionBefore,
          positionAfter,
          move,
      ...
      case "endgame":
        return analyzeEndgamePrinciple(
          principle,
          positionBefore,
          positionAfter,
          move,
      ...
      case "general":
        return analyzeGeneralPrinciple(
          principle,
          positionBefore,
          positionAfter,
          move,
    ...
    console.error(
      `Error analyzing principle ${principle.id} for move ${move}:`,
      error
    );
    return {
      violated: false,
...
function analyzeOpeningPrinciple(
  ...
  switch (principle.id) {
    ...
    case "develop-knights-bishops":
      return analyzePieceDevelopment(
        move,
        positionBefore,
        positionAfter,
        previousMoves
    ...
    case "complete-development-first":
      return analyzeCompleteDevelopment(
        move,
        positionBefore,
        positionAfter,
        previousMoves
    ...
    case "dont-bring-queen-early":
      return analyzeEarlyQueenDevelopment(
        move,
        positionBefore,
        positionAfter,
        moveNumber
    ...
    case "avoid-premature-attacks":
      return analyzePrematureAttacks(
        move,
        positionBefore,
        positionAfter,
        previousMoves
    ...
    case "develop-toward-center":
      return analyzeDevelopmentTowardCenter(
        move,
        positionBefore,
        positionAfter
      );
...
function analyzePieceDevelopment(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess,
  previousMoves: string[]
...
function analyzeTacticalThreats(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess
) {
...
function generateMoveExplanation(
  ...
  const issues = [
    ...violations,
    ...missedOpportunities.map((p) => ({
      principle: p,
      severity: "minor" as const,
      description: `Missed opportunity to apply: ${p.name}`,
      shortTermImpact: "",
      longTermImpact: "",
    })),
  ];
...
function generateOverallAssessment(moves: MoveAnalysis[]): string {
  ...
  const inaccuracies = moves.filter(
    (m) => m.assessment === "inaccurate"
  ).length;

  if (blunders > 0)
    return `Game had ${blunders} blunder(s) that need immediate attention.`;
  if (mistakes > 0)
    return `Game had ${mistakes} mistake(s) that should be reviewed.`;
  if (inaccuracies > 0)
    return `Game had ${inaccuracies} inaccuracy(ies) for improvement.`;
...
function analyzeCompleteDevelopment(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess,
  previousMoves: string[]
...
function analyzeEarlyQueenDevelopment(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess,
  moveNumber: number
...
function analyzeKingPawnWeakness(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess
) {
...
function analyzePrematureAttacks(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess,
  previousMoves: string[]
...
function analyzeDevelopmentTowardCenter(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess
) {
...
function analyzeCentralization(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess
) {
...
function analyzeTargetOpponentKing(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess
) {
...
function analyzeKingActivation(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess
) {
...
function analyzePassedPawnSupport(
  move: string,
  positionBefore: Chess,
  positionAfter: Chess
) {
