...
export interface MovePurpose {
  move: string;
  moveNumber: number;
  purpose: 'tactical' | 'positional' | 'defensive' | 'development' | 'preventive' | 'unknown';
...
async function analyzeSingleMovePurpose(
  ...
  const purpose = determineMovePurpose(move, surpriseScore, lowDepthEval, highDepthEval);
  
  // Identify tactical themes
  const tacticalThemes = identifyTacticalThemes(move, positionBefore);
  
  // Generate explanation
  const explanation = generateMoveExplanation(move, purpose, surpriseScore, bestMove, tacticalThemes, previousMoves);
...
function generateOverallAssessment(movePurposes: MovePurpose[]): string {
  const tacticalMistakes = movePurposes.filter(m => m.purpose === 'tactical' && m.surpriseScore > 1.0).length;
  const positionalMistakes = movePurposes.filter(m => m.purpose === 'positional' && m.surpriseScore > 0.5).length;
