...
export function validatePrincipleViolation(
  ...
  switch (violationType) {
    case "assess-pawn-structure":
      return validatePawnStructureViolation(
        move,
        moveDetails,
        positionBefore,
        positionAfter,
    ...
    case "maintain-king-safety":
      return validateKingSafetyViolation(
        move,
        moveDetails,
        positionBefore,
        positionAfter
    ...
    case "develop-knights-bishops":
      return validateDevelopmentViolation(
        move,
        moveDetails,
        positionBefore,
        positionAfter
...
function validatePawnStructureViolation(
  ...
  if (standardOpeningMoves.includes(move)) {
    console.log(
      `❌ Rejecting pawn structure violation for standard opening move: ${move}`
    );
    return false;
  }
  ...
  if (description.includes("doubled")) {
    ...
    if (doubledAfter <= doubledBefore) {
      console.log(
        `❌ Rejecting doubled pawn claim: ${doubledBefore} → ${doubledAfter} for move ${move}`
      );
      return false;
    }
    ...
    if (!moveDetails.captured) {
      console.log(
        `❌ Rejecting doubled pawn claim: No capture in move ${move}`
      );
      return false;
    }
  }
  ...
  if (description.includes("isolated")) {
    const isolatedBefore = countIsolatedPawns(
      positionBefore,
      moveDetails.color
    );
...
function validateKingSafetyViolation(
  ...
  if (moveDetails.piece === "n" || moveDetails.piece === "b") {
    ...
    if (moveCount < 15) {
      // Opening phase
      console.log(
        `❌ Rejecting king safety violation for development move in opening: ${move}`
      );
      return false;
    }
  }
  ...
  if (moveDetails.piece === "p") {
    ...
    if (moveCount < 10 && standardPawnMoves.includes(move)) {
      console.log(
        `❌ Rejecting king safety violation for standard pawn move: ${move}`
      );
      return false;
    }
  }
...
function validateDevelopmentViolation(
  ...
  if (moveDetails.piece === "n" || moveDetails.piece === "b") {
    ...
    if (piece) {
      ...
      if (
        startingSquares[pieceKey as keyof typeof startingSquares]?.includes(
          moveDetails.from
        )
      ) {
        console.log(
          `❌ Rejecting development violation for actual development: ${move}`
        );
        return false;
      }
    }
...
export function shouldFlagViolation(
  ...
  if (skillLevel) {
    ...
    if (
      violationType === "think-prophylactically" &&
      skillLevel.minRating < 1800
    ) {
      console.log(
        `✅ Skipping prophylactic thinking violation for ${skillLevel.name} level`
      );
      return false;
    }
    ...
    if (
      violationType === "improve-worst-piece" &&
      skillLevel.minRating < 1400
    ) {
      console.log(
        `✅ Skipping piece activity violation for ${skillLevel.name} level`
      );
      return false;
    }
    ...
    if (skillLevel.name === "Beginner") {
      ...
      if (!priorityViolations.includes(violationType)) {
        console.log(
          `✅ Skipping non-priority violation for beginner: ${violationType}`
        );
        return false;
      }
    }
