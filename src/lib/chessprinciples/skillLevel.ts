...
export function getSkillLevelFeedback(skillLevel: SkillLevel, hasViolations: boolean): string {
  const base = `Based on your accuracy (${skillLevel.name} level), `;
  
  if (skillLevel.name === 'Beginner') {
    return hasViolations 
      ? base + "focus on the fundamentals: don't hang pieces, look for basic tactics, and castle early."
      : base + "you're doing well with the basics! Keep focusing on piece safety and simple tactics.";
  }
  
  if (skillLevel.name === 'Improving') {
    return hasViolations
      ? base + "work on deeper tactical calculation and basic positional understanding."
      : base + "good tactical awareness! Now focus on positional concepts and planning.";
  }
  
  if (skillLevel.name === 'Intermediate') {
    return hasViolations
      ? base + "refine your positional understanding and strategic planning."
      : base + "solid play! Focus on subtle positional factors and deeper calculation.";
  }
  
  if (skillLevel.name === 'Advanced') {
    return hasViolations
      ? base + "work on prophylactic thinking and precise calculation."
      : base + "excellent chess! Minor refinements in calculation and positional nuances will help.";
  }
  
  return hasViolations
    ? base + "even at expert level, there's always room for perfection in calculation and evaluation."
    : base + "outstanding chess! You're playing at expert level with minimal errors.";
}
...
export function isViolationSignificant(
  ...
  const severityWeight = severity === 'major' ? 1.0 : severity === 'moderate' ? 0.7 : 0.3;
