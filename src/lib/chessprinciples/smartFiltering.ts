...
export function createAccuracyBasedFiltering(playerAccuracy: number): AccuracyBasedFiltering {
...
export function filterPrincipleViolations(
  ...
  const confidentViolations = violations.filter(violation => 
    isPrincipleViolationSound(violation, position, evaluation) &&
    getPrincipleConfidence(violation, gamePhase) >= filtering.principleConfidenceThreshold
  );
  ...
  const limitedViolations = sortedViolations.slice(0, filtering.maxViolationsPerGame);
  ...
  const assessment = determineAssessment(limitedViolations, significantOpportunities, filtering);
  
  // Generate explanation
  const explanation = generateFilteredExplanation(limitedViolations, significantOpportunities, assessment);
  
  // Determine if feedback should be shown - be more permissive
  const shouldShowFeedback = limitedViolations.length > 0 || 
...
function getPrincipleConfidence(
  ...
  const confidence = baseConfidence[violation.principle.id as keyof typeof baseConfidence] || 0.6;
...
function determineAssessment(
  ...
  if (majorViolations >= filtering.severityThresholds.mistake || totalIssues >= 3) return 'mistake';
  if (moderateViolations >= filtering.severityThresholds.inaccurate || totalIssues >= 2) return 'inaccurate';
...
function isPieceActuallyHanging(square: string, position: Chess): boolean {
  ...
  const captures = moves.filter(move => move.includes('x') && move.includes(square));
...
function isCalculationErrorActuallySignificant(position: Chess, evaluation?: PositionEval): boolean {
...
export function shouldShowMoveFeedback(
  ...
  const maxFeedbackMoves = Math.min(filtering.maxViolationsPerGame, Math.ceil(totalMoves * 0.3));
