...
// ── Synthetic insight factory ───────────────────────────────────────────────
function evalFact(over: Partial<EvalFact>): EvalFact {
  return {
    cp: 138,
    mate: null,
    depth: 14,
...
function lineFact(id: string, san: string[], pvUci: string[], ev: Partial<EvalFact>, isPlayedLine = false): LineFact {
...
function makeInsight(over: Partial<InsightContract> = {}): InsightContract {
  return {
    factIdPrefix: "M1",
    ply: 20,
    moveNumber: 11,
...
function makeContract(insights: InsightContract[]): CoachContract {
  return {
    version: "1.0",
