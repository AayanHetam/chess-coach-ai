...
export async function analyzeGameSurprises(
  ...
  console.log(
    "🎯 Starting surprise analysis for game with",
    gameHistory.length,
    "moves"
  );
  ...
  try {
    ...
    console.error(
      "❌ Failed to initialize engine service for surprise analysis:",
      error
    );
  ...
  for (let i = 0; i < gameHistory.length; i++) {
    ...
    if (isUserMove) {
      ...
      if (analysis.surpriseScore > 1.0) {
        ...
        console.log(
          `🚨 Significant surprise found: Move ${moveNumber}. ${move} (score: ${analysis.surpriseScore.toFixed(2)})`
        );
      }
    }
...
async function analyzeSingleMoveSurprise(
  ...
  console.log(
    `  🔍 Getting multi-depth evaluation for move ${moveNumber}. ${move}...`
  );

  // 1. Get multi-depth evaluation using real Stockfish engine
  const { lowDepthEval, highDepthEval, bestMove } =
    await engineService.getMultiDepthEvaluation(positionBefore);
  ...
  const movePurpose = determineMovePurposeFromSurprise(
    move,
    surpriseScore,
    lowDepthEval,
    highDepthEval
  ...
  const explanation = generateSurpriseExplanation(
    move,
    moveNumber,
    surpriseScore,
    bestMove,
...
function generateSurpriseAssessment(moves: SurpriseAnalysis[]): string {
  ...
  const moderateSurprises = moves.filter(
    (m) => m.severity === "moderate"
  ).length;
