...
export async function testMultiDepthEvaluation() {
  ...
  try {
    ...
    const { Chess } = await import('chess.js');
    const position = new Chess('rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1');
    ...
    const surpriseScore = Math.abs(result.highDepthEval - result.lowDepthEval) / 100;
