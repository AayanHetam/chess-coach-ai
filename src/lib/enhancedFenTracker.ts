...
export class EnhancedFenTracker {
  ...
  private extractPositionMetadata(fen: string): PositionData['positionMetadata'] {
  ...
  private countMaterial(piecePlacement: string): PositionData['positionMetadata']['materialCount'] {
    const counts = {
  ...
  public getPositionAtMove(moveNumber: number, halfMove: boolean = false): PositionData | null {
    if (halfMove) {
      ...
      return this.positions.find(p => p.halfMoveNumber === targetHalfMove) || null;
    }
  }
  ...
  public analyzeEvaluationChanges(): Array<{
    ...
    for (let i = 1; i < this.positions.length; i++) {
      ...
      if (!currentPos.movePlayed || !currentPos.evaluation || !previousPos.evaluation) {
        continue;
      }
   ...
   */
  public setEvaluationAtPosition(index: number, evaluation: {
    centipawns?: number;
    mate?: number;
    bestMove?: string;
    depth?: number;
