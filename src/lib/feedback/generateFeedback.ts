...
export async function generatePlayerFeedback(
  ...
  try {
    ...
    throw new Error(
      `Failed to fetch games from ${platform}. Please check the username and try again.`
    );
  }

  if (games.length === 0) {
    throw new Error(
      `No games found for username "${username}" on ${platform}.`
    );
  }
  ...
  for (const game of gamesToAnalyze) {
    try {
      ...
      const gameId =
        "id" in game ? game.id : "uuid" in game ? game.uuid : "unknown";
  ...
  const principleViolations = analyzePrincipleViolations(
    allPrincipleViolations,
    gameAnalyses
  );
...
function calculateAverageRating(gameAnalyses: GameAnalysis[]): number {
  const gamesWithRatings = gameAnalyses.filter(
    (game) => game.opponentRating > 0
  );
...
function generateOpeningSuggestions(opening: string, stats: any): string[] {
  ...
  if (stats.winRate < 40) {
    suggestions.push(
      "Consider studying this opening more deeply or exploring alternatives"
    );
  }

  if (stats.total >= 3 && stats.winRate < 50) {
    suggestions.push(
      "Review your games in this opening to identify common mistakes"
    );
  }

  if (opening === "Unknown") {
    suggestions.push(
      "Study basic opening principles to improve your early game"
    );
  }
    ...
    for (const game of gameAnalyses) {
      if (game.principleViolations.some((v) => v.principle.id === key)) {
        ...
        const gameViolation: GameViolation = {
          ...
          result: game.result,
          date:
            typeof game.date === "string" ? game.date : game.date.toISOString(),
          pgn: game.pgn,
        ...
        if (
          !stats.games.some(
            (g) => g.gameId === game.gameId && g.moveNumber === moveNumber
          )
        ) {
          stats.games.push(gameViolation);
        }
...
function generateImpactDescription(principle: any, frequency: number): string {
  const impactMap: Record<string, string> = {
    "dont-hang-pieces":
      "Material losses significantly impact your winning chances",
    "control-center-pawns":
      "Lack of central control limits your strategic options",
    "develop-knights-bishops":
      "Slow development gives your opponent the initiative",
    "castle-early": "Exposed king makes you vulnerable to attacks",
    "look-for-basic-tactics":
      "Missing tactical opportunities costs material and position",
    "control-space":
      "Lack of space control limits piece mobility and attacking chances",
    "assess-pawn-structure": "Poor pawn structure creates long-term weaknesses",
    "maintain-material-balance":
      "Unnecessary material sacrifices reduce winning chances",
    "avoid-moving-same-piece":
      "Inefficient piece movement wastes valuable time",
    "develop-all-pieces": "Undeveloped pieces limit your attacking potential",
  };

  const baseImpact =
    impactMap[principle.id] ||
    "This principle violation affects your overall game quality";
...
function generateImprovementTip(principle: any): string {
  const tipMap: Record<string, string> = {
    "dont-hang-pieces":
      "Before every move, check if any of your pieces are under attack",
    "control-center-pawns":
      "Aim to control central squares with pawns in the opening",
    "develop-knights-bishops":
      "Develop your minor pieces before moving pawns unnecessarily",
    "castle-early": "Castle within the first 10 moves to ensure king safety",
    "look-for-basic-tactics":
      "Scan for forks, pins, and discovered attacks before each move",
    "control-space":
      "Occupy central squares and expand on the side where you have more space",
    "assess-pawn-structure":
      "Avoid creating isolated, doubled, or backward pawns",
    "maintain-material-balance":
      "Only sacrifice material for clear tactical or positional gains",
    "avoid-moving-same-piece":
      "Develop all pieces before moving the same piece multiple times",
    "develop-all-pieces":
      "Get all your pieces into the game before launching attacks",
  };

  return (
    tipMap[principle.id] || "Study this principle and practice it in your games"
  );
}
...
function generateImprovementAreas(
  ...
  // Analyze by category
  const categories = {
    opening: principleViolations.filter(
      (v) => v.principle.category === "opening"
    ),
    middlegame: principleViolations.filter(
      (v) => v.principle.category === "middlegame"
    ),
    endgame: principleViolations.filter(
      (v) => v.principle.category === "endgame"
    ),
    tactics: principleViolations.filter(
      (v) => v.principle.id === "look-for-basic-tactics"
    ),
    strategy: principleViolations.filter(
      (v) => v.principle.category === "general"
    ),
  };
  ...
  if (categories.opening.length > 0) {
    areas.push({
      ...
      description:
        "Focus on controlling the center, developing pieces, and ensuring king safety",
      priority: categories.opening.some((v) => v.severity === "major")
        ? "high"
        : "medium",
      specificGames: categories.opening.flatMap((v) =>
        v.games.map((g) => g.gameId)
      ),
      exercises: [
  ...
  if (categories.tactics.length > 0) {
    areas.push({
      ...
      description:
        "Improve your ability to spot and calculate tactical opportunities",
      priority: "high",
      specificGames: categories.tactics.flatMap((v) =>
        v.games.map((g) => g.gameId)
      ),
      exercises: [
  ...
  if (categories.strategy.length > 0) {
    areas.push({
      ...
      description:
        "Develop better understanding of positional play and long-term planning",
      priority: "medium",
      specificGames: categories.strategy.flatMap((v) =>
        v.games.map((g) => g.gameId)
      ),
      exercises: [
