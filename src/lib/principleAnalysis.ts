...
export class PrincipleAnalysisEngine {
   ...
   */
  async analyzePositionAtDepths(
    fen: string,
    depths: number[] = [5, 10, 15, 20, 25]
  ...
  findCriticalMoves(analyses: DepthAnalysis[]): CriticalMoveAnalysis | null {
    ...
    for (let i = 1; i < analyses.length; i++) {
      const evalChange = Math.abs(
        analyses[i].evaluation - analyses[i - 1].evaluation
      );
  ...
  private detectPrincipleViolations(
    ...
    if (features.isolatedPawns > 2) {
      violations.push({
        ...
        suggestedCorrection:
          "Maintain pawn structure and avoid creating isolated pawns",
    ...
    if (features.undevelopedPieces > 3) {
      violations.push({
        ...
        suggestedCorrection:
          "Develop pieces to active squares early in the game",
        fenEvidence: [position.fen],
      });
    }
    ...
    if (features.kingSafety < 0.3) {
      violations.push({
        ...
        suggestedCorrection:
          "Castle early and maintain pawn shield around the king",
        fenEvidence: [position.fen],
      });
    }
   ...
   */
  private async getStockfishAnalysis(
    fen: string,
    depth: number
  ): Promise<DepthAnalysis> {
