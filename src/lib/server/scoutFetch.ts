...
export type ScoutResponsePayload = {
  username: string;
...
// Module-scoped in-memory LRU. Node runtime only; survives across requests
// within a single serverless container. Declared at module level so the
// reference is stable across calls — do NOT relocate inside a function.
...
/**
 * Public API: fetch a player's recent games from Chess.com or Lichess, with
 * the module-scoped 10-min cache applied. Returns the same payload shape that
 * /api/scout/route.ts previously returned, including the `cached` flag on hits.
 *
 * Used by both /api/scout/route.ts (HTTP entry) and src/lib/mastermind/wireValidators.ts
 * (Stage B internal caller). Sharing one cache across both call sites is the
 * point — see PR_1C_STAGE_B_PLAN.md §12.3 T14 + cache-path resolution.
 */
...
/**
 * Test-only seam: clear the module-scoped cache between unit tests so that
 * each test starts from a clean slate. Not for production callers.
 */
export function __resetScoutCacheForTesting(): void {
  scoutCache.clear();
}
